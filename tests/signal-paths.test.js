...
function buildSignalGraph({ inputs, mixEffects, downstreamKeyers = [], routing = [] }) {
  ...
  const directUses = id => {
    ...
    mixEffects.forEach(me => {
      ...
      (me.upstreamKeyers || []).forEach(k => {
        if (Number(k.fillId) === Number(id) || Number(k.keyId) === Number(id)) out.push({ kind:'direct', label:'M/E '+me.index+' · USK '+k.index+' · '+(Number(k.fillId)===Number(id)?'FILL':'KEY') });
      });
    });
    downstreamKeyers.forEach(k => {
      if (Number(k.fillId) === Number(id) || Number(k.keyId) === Number(id)) out.push({ kind:'direct', label:'DSK '+k.index+' · '+(Number(k.fillId)===Number(id)?'FILL':'KEY') });
    });
    routing.forEach(r => {
      if (Number(r.sourceId) === Number(id)) out.push({ kind:'route', label:r.name });
    });
    return out;
  };
  const descendants = (id, visited = new Set()) => {
    ...
    const next = new Set(visited); next.add(n);
